		...
		<div class="conteneur panneau-tirage-audio">
			<div class="contenu" v-if="mode === 'edition'">
				...
				<div class="zone" @dragover.prevent @drop.prevent>
					<input :id="'selectionner_audios_' + id" type="file" multiple accept="audio/*" @change="selectionnerAudios" style="display: none;">
					<label :for="'selectionner_audios_' + id" tabindex="0" @drop="glisserAudios" @keydown.enter="activerInput('selectionner_audios_' + id)">{{ $t('selectionnerAudios') }}</label>
				</div>
				<div class="audios">
					<div class="audio" v-for="(audio, index) in audios" :key="'audio_' + index">
						<span class="item" role="button" tabindex="0" :class="{'lecture': lecture === audio.id}" @click="lireAudio(audio.donnees, audio.id)" @keydown.enter="lireAudio(audio.donnees, audio.id)"><i class="material-icons">play_arrow</i></span>
						<span class="fermer" role="button" tabindex="0" @click="supprimerAudio(index, audio.id)" @keydown.enter="supprimerAudio(index, audio.id)"><i class="material-icons">highlight_off</i></span>
					</div>
				</div>
			...
			<div class="contenu lecture" v-else>
				<div class="tirage" v-if="tirage !== 'tirage' && !tirageEnCours">
					<span class="item" role="button" tabindex="0" :class="{'lecture': lecture === tirage.id}" @click="lireAudioTirage(tirage.id)" @keydown.enter="lireAudioTirage(tirage.id)"><i class="material-icons">play_arrow</i></span>
...
export default {
	...
	mounted () {
		...
		if (this.mode === 'edition' && this.statut !== 'min') {
			setTimeout(function () {
				document.querySelector('label[for="selectionner_audios_' + this.id + '"]').focus()
			}.bind(this), 0)
		...
		tirer () {
			...
			setTimeout(function () {
				...
				this.$nextTick(function () {
					...
					document.querySelector('#' + this.id + ' .item').focus()
				}.bind(this))
			}.bind(this), 1200)
		},
		editer () {
