	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-webcam">
			<div class="contenu" v-if="mode === 'edition'">
				<label :for="'webcam_' + id">{{ $t('webcam') }}</label>
				<div class="webcam">
					<select :id="'webcam_' + id" @change="webcam = $event.target.value">
			...
			<div class="contenu" v-else>
				<video :id="'video_' + id" controls autoplay />
			</div>
		</div>
...
export default {
	...
	mounted () {
		...
		if (this.mode === 'edition' && this.statut !== 'min') {
			setTimeout(function () {
				document.querySelector('#' + this.id + ' select').focus()
			}.bind(this), 0)
		}
		if (this.webcam !== '') {
	...
	methods: {
		generer () {
				...
				if (!navigator.mediaDevices?.getUserMedia) {
					...
					navigator.mediaDevices.getUserMedia({ audio: true, video: true }).then(function (flux) {
						this.$nextTick(function () {
							document.querySelector('#video_' + this.id).srcObject = flux
						}.bind(this))
					}.bind(this))
				}
			} else {
			...
			this.$nextTick(function () {
				document.querySelector('#video_' + this.id).focus()
			}.bind(this))
			this.positionner()
		},
		editer () {
