	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-plateau" v-if="mode === 'edition'">
			<div class="contenu" v-if="!chargementImage">
				...
				<label :for="'selectionner_plateau_' + id" class="bouton" role="button" tabindex="0" @keydown.enter="activerInput('selectionner_plateau_' + id)">{{ $t('selectionnerImage') }}</label>
				<input :id="'selectionner_plateau_' + id" type="file" @change="televerserImage" style="display: none" accept=".jpg, .jpeg, .png, .gif">
				<label :for="'nombre_pions_' + id">{{ $t('nombrePions') }}</label>
				<input :id="'nombre_pions_' + id" type="number" :value="pions" :min="2" :max="12" @input="pions = parseInt($event.target.value)">
		...
		<div class="conteneur panneau-plateau" v-else>
			<div class="conteneur-plateau">
				...
				<div class="pions">
					<img :id="'pion_' + pion + '_' + id" class="pion" v-for="pion in pions" :src="'./static/img/plateau/pion' + pion + '.png'" :key="'pion_' + pion + '_' + id" :alt="'pion_' + pion">
				</div>
			</div>
		</div>
