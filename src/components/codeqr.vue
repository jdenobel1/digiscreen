<template>
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-codeqr">
			<div class="contenu">
				<label :for="'lien_' + id" v-if="mode === 'edition'">{{ $t('lienOuTexte') }}</label>
				<input :id="'lien_' + id" type="search" :maxlength="255" :value="texte" @input="texte = $event.target.value" v-if="mode === 'edition'">
				...
				<span class="bouton" role="button" tabindex="0" @click="generer" @keydown.enter="generer" v-if="mode === 'edition'">{{ $t('valider') }}</span>
			</div>
		</div>
...
.panneau .conteneur .codeqr canvas {
