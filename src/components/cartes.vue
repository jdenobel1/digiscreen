	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-cartes" v-if="mode === 'edition'">
			<div class="contenu">
				...
				<div class="format">
					<span class="paysage">
						<input :id="'format_paysage_' + id" type="radio" :name="'format_' + id" value="paysage" :checked="format === 'paysage'" @change="format = $event.target.value">
						<label :for="'format_paysage_' + id" tabindex="0" @keydown.enter="activerInput('format_paysage_' + id)">{{ $t('paysage') }}</label>
					</span>
					<span class="portrait">
						<input :id="'format_portrait_' + id" type="radio" :name="'format_' + id" value="portrait" :checked="format === 'portrait'" @change="format = $event.target.value">
						<label :for="'format_portrait_' + id" tabindex="0" @keydown.enter="activerInput('format_portrait_' + id)">{{ $t('portrait') }}</label>
					</span>
					<span class="carre">
						<input :id="'format_carre_' + id" type="radio" :name="'format_' + id" value="carre" :checked="format === 'carre'" @change="format = $event.target.value">
						<label :for="'format_carre_' + id" tabindex="0" @keydown.enter="activerInput('format_carre_' + id)">{{ $t('carre') }}</label>
					</span>
				</div>
				<label>{{ $t('recto') }}</label>
				<div class="conteneur-recto">
					...
					<input :id="'fond-recto-' + id" type="color" :value="fondRecto" :title="$t('selectionnerCouleurFond')" @change="fondRecto = $event.target.value" v-if="imageRecto === '' && chargementImage === ''">
					<span class="fond-recto" v-if="fondRecto === '#001d1d' && imageRecto === '' && chargementImage === ''">
						<label :for="'televerser_image_recto_' + id" class="bouton-secondaire" :title="$t('selectionnerImage')" role="button" tabindex="0" @keydown.enter="activerInput('televerser_image_recto_' + id)"><i class="material-icons">add_photo_alternate</i></label>
						<input :id="'televerser_image_recto_' + id" type="file" @change="televerserImage($event, 'recto')" style="display: none" accept=".jpg, .jpeg, .png, .gif">
					</span>
					...
					<span class="supprimer-image" role="button" tabindex="0" @click="imageRecto = ''" @keydown.enter="imageRecto = ''" v-if="imageRecto !== ''"><i class="material-icons">highlight_off</i></span>
					<span class="supprimer-fond" role="button" tabindex="0" @click="fondRecto = '#001d1d'" @keydown.enter="fondRecto = '#001d1d'" v-if="fondRecto !== '#001d1d' && imageRecto === ''"><i class="material-icons">highlight_off</i></span>
				</div>
				<label>{{ $t('verso') }}</label>
				<div class="conteneur-verso">
					...
					<input type="color" :id="'fond_verso_' + id" :value="fondVerso" :title="$t('selectionnerCouleurFond')" @change="fondVerso = $event.target.value" v-if="imageVerso === '' && chargementImage === ''">
					<span class="fond-verso" v-if="fondVerso === '#ffffff' && imageVerso === '' && chargementImage === ''">
						<label :for="'televerser_image_verso_' + id" class="bouton-secondaire" :title="$t('selectionnerImage')" role="button" tabindex="0" @keydown.enter="activerInput('televerser_image_verso_' + id)"><i class="material-icons">add_photo_alternate</i></label>
						<input :id="'televerser_image_verso_' + id" type="file" @change="televerserImage($event, 'verso')" style="display: none" accept=".jpg, .jpeg, .png, .gif">
					</span>
					...
					<span class="supprimer-image" role="button" tabindex="0" @click="imageVerso = ''" @keydown.enter="imageVerso = ''" v-if="imageVerso !== ''"><i class="material-icons">highlight_off</i></span>
					<span class="supprimer-fond" role="button" tabindex="0" @click="fondVerso = '#ffffff'" @keydown.enter="fondVerso = '#ffffff'" v-if="fondVerso !== '#ffffff' && imageVerso === ''"><i class="material-icons">highlight_off</i></span>
				</div>
				<span class="bouton-secondaire ajouter" role="button" tabindex="0" :title="$t('ajouterCarte')" @click="ajouterCarte" @keydown.enter="ajouterCarte"><i class="material-icons">add_circle_outline</i></span>
				<draggable class="items" v-model="cartes">
					<div class="item" :class="format" v-for="(carte, index) in cartes" :key="'item_' + index">
						<div :id="'item_' + id + '_' + index" class="carte" role="button" tabindex="0" @click="retournerCarte('item_' + id + '_' + index)" @keydown.enter="retournerCarte('item_' + id + '_' + index)">
						...
						<span class="fermer" role="button" tabindex="0" @click="supprimerCarte(index)" @keydown.enter="supprimerCarte(index)"><i class="material-icons">highlight_off</i></span>
					</div>
				</draggable>
		...
		<div class="conteneur panneau-cartes" v-else>
			<div class="contenu cartes">
				<div :id="'carte_' + id + '_' + index" class="carte" role="button" tabindex="0" :class="format" v-for="(carte, index) in cartes" @click="retournerCarte('carte_' + id + '_' + index)" @keydown.enter="retournerCarte('carte_' + id + '_' + index)" :key="'carte_' + index">
					<div class="recto" :class="{'avec-image': carte.recto.image !== ''}" :style="'background-color:' + carte.recto.fond">
						<div :style="'background-image: url(' + carte.recto.image + ')'" v-if="carte.recto.image !== ''">
							<h1 :style="'color:' + definirCouleurTexte(carte.recto.fond)" v-if="carte.recto.texte !== ''">{{ carte.recto.texte }}</h1>
						</div>
...
export default {
	...
	mounted () {
		...
		if (this.mode === 'edition' && this.statut !== 'min') {
			...
			setTimeout(function () {
				document.querySelector('#' + this.id + ' .carte').focus()
			}.bind(this), 0)
		}
	},
	methods: {
...
.panneau .panneau-cartes input[type="color"]:focus::-moz-color-swatch {
	border: 1px solid #001d1d;
}
...
.panneau .panneau-cartes input[type="color"]:focus::-webkit-color-swatch {
	border: 1px solid #001d1d;
}
