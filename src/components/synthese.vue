	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-synthese">
			<div class="contenu" v-if="mode === 'edition'">
				...
				<label>{{ $t('voix') }}</label>
				<div class="voix">
					<input type="text" :list="'liste_voix_' + id" :value="voix" @input="voix = $event.target.value" :disabled="listeVoix.length === 0" />
					<datalist :id="'liste_voix_' + id">
						<option value="">--</option>
						<template v-for="(item, index) in listeVoix">
							<option :value="item.name" v-if="item.default" :key="'voix_defaut_' + index">{{ item.name }} ({{ item.lang }}) -- {{ $t('parDefaut') }}</option>
							<option :value="item.name" v-else :key="'voix_' + index">{{ item.name }} ({{ item.lang }})</option>
						</template>
					</datalist>
				</div>
				<div class="actions">
					<span class="bouton" role="button" tabindex="0" @click="parler" @keydown.enter="parler">{{ $t('ecouter') }}</span>
					<span class="bouton" role="button" tabindex="0" @click="generer" @keydown.enter="generer">{{ $t('valider') }}</span>
				</div>
			</div>
			<span class="bouton audio" role="button" tabindex="0" @click="parler" @keydown.enter="parler" v-else><i class="material-icons">play_arrow</i></span>
		</div>
...
export default {
	...
	mounted () {
		...
		if (this.mode === 'edition' && this.listeVoix.length === 0) {
			...
			setTimeout(function () {
				document.querySelector('#' + this.id + ' .bouton.audio').focus()
			}.bind(this), 0)
		}
	},
	methods: {
