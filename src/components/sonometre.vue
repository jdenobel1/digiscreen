		...
		<div class="conteneur panneau-sonometre">
			<div class="contenu">
				...
				<div class="sensibilite">
					<span role="button" tabindex="0" @click="modifierSensibilite(98)" @keydown.enter="modifierSensibilite(98)" :class="{'selectionne': sensibilite === 98 }">1</span>
					<span role="button" tabindex="0" @click="modifierSensibilite(88)" @keydown.enter="modifierSensibilite(88)" :class="{'selectionne': sensibilite === 88 }">2</span>
					<span role="button" tabindex="0" @click="modifierSensibilite(78)" @keydown.enter="modifierSensibilite(78)" :class="{'selectionne': sensibilite === 78 }">3</span>
					<span role="button" tabindex="0" @click="modifierSensibilite(68)" @keydown.enter="modifierSensibilite(68)" :class="{'selectionne': sensibilite === 68 }">4</span>
					<span role="button" tabindex="0" @click="modifierSensibilite(58)" @keydown.enter="modifierSensibilite(58)" :class="{'selectionne': sensibilite === 58 }">5</span>
					<span role="button" tabindex="0" @click="modifierSensibilite(48)" @keydown.enter="modifierSensibilite(48)" :class="{'selectionne': sensibilite === 48 }">6</span>
					<span role="button" tabindex="0" @click="modifierSensibilite(38)" @keydown.enter="modifierSensibilite(38)" :class="{'selectionne': sensibilite === 38 }">7</span>
				</div>
				<label>{{ $t('seuilTolerance') }}</label>
				<input type="number" :value="tolerance" @input="tolerance = $event.target.value" :min="1" :max="100">
				<div class="actions">
					<span class="bouton" role="button" tabindex="0" @click="demarrer" @keydown.enter="demarrer" v-if="verificationVolume === ''">{{ $t('demarrer') }}</span>
					<span class="bouton" role="button" tabindex="0" @click="pause" @keydown.enter="pause" v-else>{{ $t('pause') }}</span>
				</div>
			</div>
		</div>
	...
	watch: {
		...
		volume: function (valeur) {
			if (valeur >= this.sensibilite) {
				this.decompte++
				if (this.decompte === this.tolerance * 10) {
