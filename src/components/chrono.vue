				...
				<div class="options" v-if="mode === 'stop'">
					...
					<div class="conteneur-affichage-millisecondes">
						...
						<div class="affichage-millisecondes" >
							<span class="oui">
								<input :id="'millisecondes_oui_' + id" type="radio" :name="'millisecondes_oui_' + id" value="oui" :checked="affichageMillisecondes=== 'oui'" @change="modifierAffichageMillisecondes($event.target.value)">
								<label :for="'millisecondes_oui_' + id" tabindex="0" @keydown.enter="activerInput('millisecondes_oui_' + id)">{{ $t('oui') }}</label>
							</span>
							<span class="non">
								<input :id="'millisecondes_non_' + id" type="radio" :name="'millisecondes_oui_' + id" value="non" :checked="affichageMillisecondes=== 'non'" @change="modifierAffichageMillisecondes($event.target.value)">
								<label :for="'millisecondes_non_' + id" tabindex="0" @keydown.enter="activerInput('millisecondes_non_' + id)">{{ $t('non') }}</label>
							</span>
						</div>
					</div>
				</div>
				<div class="actions">
					<span class="bouton" role="button" tabindex="0" @click="demarrer" @keydown.enter="demarrer" v-if="mode === 'stop'">{{ $t('demarrer') }}</span>
					<span class="bouton" role="button" tabindex="0" @click="demarrer" @keydown.enter="demarrer" v-else-if="mode === 'pause'"><i class="material-icons">play_arrow</i></span>
					<span class="bouton" role="button" tabindex="0" @click="pause" @keydown.enter="pause" v-else-if="mode === 'lecture'"><i class="material-icons">pause</i></span>
					<span class="bouton" role="button" tabindex="0" @click="stop" @keydown.enter="stop" v-if="mode !== 'stop'"><i class="material-icons">autorenew</i></span>
				</div>
			</div>
		</div>
