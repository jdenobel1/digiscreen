	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-surlignage">
			<div class="contenu" v-if="mode === 'edition'">
				<label :for="'texte_' + id">{{ $t('texte') }}</label>
				<textarea :id="'texte_' + id" class="surlignage" :value="texte" @input="texte = $event.target.value"></textarea>
			...
			<div class="contenu" v-else>
				<div class="surligneurs">
					<span class="surligneur jaune" role="button" tabindex="0" :class="{'actif': surligneur === '#faff00'}" @click="surligneur = '#faff00'" @keydown.enter="surligneur = '#faff00'" />
					<span class="surligneur vert" role="button" tabindex="0" :class="{'actif': surligneur === '#00ff00'}" @click="surligneur = '#00ff00'" @keydown.enter="surligneur = '#00ff00'" />
					<span class="surligneur rose" role="button" tabindex="0" :class="{'actif': surligneur === '#fb3bc3'}" @click="surligneur = '#fb3bc3'" @keydown.enter="surligneur = '#fb3bc3'" />
					<span class="surligneur bleu" role="button" tabindex="0" :class="{'actif': surligneur === '#04fdff'}" @click="surligneur = '#04fdff'" @keydown.enter="surligneur = '#04fdff'" />
				</div>
				<div class="surlignage" :class="{'plein-ecran': statut === 'max'}">
					<template v-for="(item, index) in items">
						<template v-if="item !== '<br>' && item !== ''">
							<span tabindex="0" :style="{'background': definirCouleur(index)}" @click="surligner(index)" @keydown.enter="surligner(index)" v-if="itemsSurlignes.map(function (i) { return i.index }).includes(index)" :key="'item_surligne_' + index + '_' + item">{{ item }}</span>
							<span tabindex="0" @click="surligner(index)" @keydown.enter="surligner(index)" v-else :key="'item_' + index + '_' + item">{{ item }}</span>
						</template>
