	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-lien" v-if="mode === 'edition'">
			<div class="contenu" v-show="!chargementIcone">
				<label :for="'lien_' + id">{{ $t('lien') }}</label>
				<input :id="'lien_' + id" type="search" class="lien" :value="lien" @input="lien = $event.target.value" @keydown.enter="generer">
				<label :for="'titre_lien_' + id">{{ $t('titreLien') }}</label>
				<input :id="'titre_lien_' + id" type="search" class="titre-lien" :value="titreLien" @input="titreLien = $event.target.value" @keydown.enter="generer">
...
export default {
	...
	mounted () {
		...
		if (this.mode === 'edition' && this.statut !== 'min' && this.lien === '') {
			...
			setTimeout(function () {
				document.querySelector('#' + this.id + ' a').focus()
			}.bind(this), 0)
		}
	},
	methods: {
