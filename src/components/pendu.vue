	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-pendu" v-if="mode === 'edition'">
			<div class="contenu" v-if="!chargementImage">
				<label :for="'mot_' + id">{{ $t('motATrouver') }}</label>
				<input :id="'mot_' + id" type="search" :value="mot" @input="mot = $event.target.value">
				<label>{{ $t('image') }}</label>
				...
				<label :for="'nombre_erreurs_' + id">{{ $t('nombreErreurs') }}</label>
				<input :id="'nombre_erreurs_' + id" type="number" :value="erreurs" :min="1" :max="20" @input="erreurs = parseInt($event.target.value)">
				<label :for="'caracteres_' + id">{{ $t('caracteres') }}</label>
				<textarea :id="'caracteres_' + id" class="caracteres" :value="caracteres" @input="caracteres = $event.target.value"></textarea>
			...
			<div class="contenu">
				...
				<div class="droite">
					...
					<div class="caracteres">
						<span class="caractere" role="button" tabindex="0" :class="{'inactif': motTrouve || erreursRestantes === 0}" v-for="(caractere, indexCaractere) in tableauCaracteres" @click="verifierLettre(caractere)" @keydown.enter="verifierLettre(caractere)" :key="'caractere_' + indexCaractere">{{ caractere.toUpperCase() }}</span>
					</div>
				</div>
			</div>
		</div>
