<template>
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-iframe" v-if="mode === 'edition' && !chargementIframe">
			<div class="contenu">
				<label :for="'lien_contenu_' + id">{{ $t('lienContenuLigne') }}</label>
				<input :id="'lien_contenu_' + id" type="search" :value="lien" @input="lien = $event.target.value" @keydown.enter="generer">
		...
		<div class="conteneur panneau-iframe large" v-else>
			<iframe :src="iframe" allow="autoplay; fullscreen" crossorigin></iframe>
		</div>
