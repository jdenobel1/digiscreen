<template>
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-galerie" :class="{'mode-lecture': mode === 'lecture'}">
			<div class="contenu" v-if="mode === 'edition'">
				<label>{{ $t('texteGalerie') }}</label>
				<div class="zone" @dragover.prevent @drop.prevent>
					<input :id="'selectionner_images_' + id" type="file" multiple accept="image/*" @change="selectionnerImages" style="display: none;">
					<label :for="'selectionner_images_' + id" tabindex="0" @drop="glisserImages" @keydown.enter="activerInput('selectionner_images_' + id)">{{ $t('selectionnerImages') }}</label>
				</div>
				<draggable class="images" v-model="images">
					<div class="image" v-for="(image, index) in images" :key="'image_' + index">
						<img :src="image" :alt="'Image' + index">
						<span class="fermer" role="button" tabindex="0" @click="supprimerImage(index)" @keydown.enter="supprimerImage(index)"><i class="material-icons">highlight_off</i></span>
					</div>
				</draggable>
			...
			<div class="contenu lecture" v-else>
				<div class="galerie" tabindex="0" @keydown="gererClavierGalerie">
					...
					<span class="diapo-precedente" role="button" tabindex="0" @click="afficherDiapoPrecedente" @keydown.enter="afficherDiapoPrecedente"><i class="material-icons">navigate_before</i></span>
					<span class="diapo-suivante" role="button" tabindex="0" @click="afficherDiapoSuivante" @keydown.enter="afficherDiapoSuivante"><i class="material-icons">navigate_next</i></span>
				</div>
			</div>
		</div>
...
export default {
	...
	mounted () {
		...
		if (this.mode === 'lecture' && this.statut !== 'min') {
			setTimeout(function () {
				document.querySelector('#' + this.id + ' .galerie').focus()
			}.bind(this), 0)
		}
	},
	methods: {
...
<style>
.panneau .panneau-galerie.mode-lecture {
