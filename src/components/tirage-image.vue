<template>
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-tirage-image" :class="{'tirage-image': mode === 'lecture'}">
...
export default {
	...
	mounted () {
		...
		if (this.mode === 'edition' && this.statut !== 'min') {
			setTimeout(function () {
				document.querySelector('label[for="selectionner_images_' + this.id + '"]').focus()
			}.bind(this), 0)
...
<style>
.panneau .panneau-tirage-image.tirage-image {
...
.focus .panneau .panneau-tirage-image div.tirage {
