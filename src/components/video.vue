	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-video">
			<div class="contenu" v-if="mode === 'edition' && !chargementVideo">
				<label :for="'lien_video_' + id">{{ $t('lienVideo') }}</label>
				<div class="valider">
					<input :id="'lien_video_' + id" type="search" :value="lien" @input="lien = $event.target.value" @keydown.enter="valider">
					<span class="bouton-secondaire" role="button" tabindex="0" :title="$t('valider')" @click="valider" @keydown.enter="valider"><i class="material-icons">search</i></span>
				</div>
				<div class="separateur"><span>{{ $t('ou') }}</span></div>
				<label>{{ $t('fichierAppareil') }}</label>
				<label :for="'selectionner_video_' + id" class="bouton" role="button" tabindex="0" @keydown.enter="activerInput('selectionner_video_' + id)">{{ $t('selectionnerVideo') }}</label>
				<input :id="'selectionner_video_' + id" type="file" @change="televerserVideo" style="display: none" accept=".mp4, .m4v, .ogv">
			</div>
			...
			<div class="contenu" v-else-if="mode === 'decoupage'">
				<div class="video">
					<iframe :src="'https://www.youtube-nocookie.com/embed/' + videoId + '?iv_load_policy=3&modestbranding=1&rel=0&showinfo=0&version=3'" allow="autoplay; fullscreen" crossorigin frameborder="0" v-if="debut === 0 && fin === 0" />
					<iframe :src="'https://www.youtube-nocookie.com/embed/' + videoId + '?start=' + debut + '&end=' + fin + '&iv_load_policy=3&modestbranding=1&rel=0&showinfo=0&version=3'" allow="autoplay; fullscreen" crossorigin frameborder="0" v-else-if="debut > 0 && fin > 0" />
					<iframe :src="'https://www.youtube-nocookie.com/embed/' + videoId + '?start=' + debut + '&iv_load_policy=3&modestbranding=1&rel=0&showinfo=0&version=3'" allow="autoplay; fullscreen" crossorigin frameborder="0" v-else-if="debut > 0" />
					<iframe :src="'https://www.youtube-nocookie.com/embed/' + videoId + '?end=' + fin + '&iv_load_policy=3&modestbranding=1&rel=0&showinfo=0&version=3'" allow="autoplay; fullscreen" crossorigin frameborder="0" v-else />
				</div>
				<div class="temps">
					<div class="debut">
						<label>{{ $t('debut') }}</label>
						<div class="minutes-secondes">
							<div class="minutes">
								<label :for="'minutes_debut_' + id">{{ $t('min') }}</label>
								<input :id="'minutes_debut_' + id" type="number" :value="debutMinutes" :min="0" @input="debutMinutes = $event.target.value">
							</div>
							<div class="secondes">
								<label :for="'secondes_debut_' + id">{{ $t('sec') }}</label>
								<input :id="'secondes_debut_' + id" type="number" :value="debutSecondes" :min="0" :max="59" @input="debutSecondes = $event.target.value">
							</div>
						</div>
					</div>
					<div class="fin">
						...
						<div class="minutes-secondes">
							<div class="minutes">
								<label :for="'minutes_fin_' + id">{{ $t('min') }}</label>
								<input :id="'minutes_fin_' + id" type="number" :value="finMinutes" :min="0" @input="finMinutes = $event.target.value">
							</div>
							<div class="secondes">
								<label :for="'secondes_fin_' + id">{{ $t('sec') }}</label>
								<input :id="'secondes_fin_' + id" type="number" :value="finSecondes" :min="0" :max="59" @input="finSecondes = $event.target.value">
							</div>
						</div>
					</div>
				</div>
				<div class="actions">
					<span class="bouton" role="button" tabindex="0" @click="decouper" @keydown.enter="decouper">{{ $t('apercu') }}</span>
			...
			<template v-else-if="mode === 'lecture' && videoId === '' && lien.includes('https://vimeo.com')">
				<iframe :src="'https://player.vimeo.com/video/' + lien.replace('https://vimeo.com/', '')" allow="autoplay; fullscreen" crossorigin frameborder="0" />
			</template>
	...
	methods: {
		valider () {
			if (this.verifierURL(this.lien) === true) {
				...
				const regexYTId = /^.*(youtu\.be\/|v\/|u\/\w\/|embed\/|shorts\/|watch\?v=|&v=)([^#&?]*).*/
				...
				this.$nextTick(function () {
					this.lecteur = videojs('#' + this.id + ' .video-js', { language: this.$parent.langue, languages: { fr, es, it, de, nl, hr, en }, playbackRates: [0.6, 0.7, 0.8, 0.9, 1, 1.1, 1.2, 1.3, 1.4] }, function onPlayerReady () {
						setTimeout(function () {
							...
							document.querySelector('#' + this.id + ' .video-js .vjs-big-play-button').focus()
						}.bind(this), 100)
					}.bind(this))
				}.bind(this))
			}
...
.panneau .panneau-video .video iframe {
	width: 100%;
}
</style>
