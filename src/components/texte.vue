	...
	methods: {
		...
		creer () {
			...
			this.editeur.onpaste = function (event) {
				...
				if (html !== '') {
					html = html.replace(/<style([\S\s]*?)>([\S\s]*?)<\/style>/mgi, '')
					html = stripTags(html, ['b', 'i', 'u', 'strike', 'a', 'br', 'div', 'font', 'ul', 'ol'])
					html = html.replace(/style=".*?"/mgi, '')
					html = html.replace(/class=".*?"/mgi, '')
			...
			document.querySelector('#couleur-texte-' + this.id).closest('button').setAttribute('tabindex', '-1')
			document.querySelector('#couleur-fond-' + this.id).closest('button').setAttribute('tabindex', '-1')
		},
...
.pell-button input[type="color"]:focus::-moz-color-swatch {
...
.pell-button input[type="color"]:focus::-webkit-color-swatch {
	border: 1px solid #001d1d;
}
