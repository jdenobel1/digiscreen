<template>
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-image" :class="{'mode-lecture': mode === 'lecture'}">
			<div class="contenu" v-if="mode === 'edition' && !chargementImage">
				...
				<div class="rechercher" v-if="pixabayAPIKey !== ''">
					...
					<span role="button" tabindex="0" :title="$t('rechercher')" class="bouton-secondaire" @click="generer" @keydown.enter="generer"><i class="material-icons">search</i></span>
				</div>
				<div class="separateur" v-if="pixabayAPIKey !== ''"><span>{{ $t('ou') }}</span></div>
				<label>{{ $t('fichierAppareil') }}</label>
				<label :for="'selectionner_image_' + id" class="bouton" role="button" tabindex="0" @keydown.enter="activerInput('selectionner_image_' + id)">{{ $t('selectionnerImage') }}</label>
				<input :id="'selectionner_image_' + id" type="file" @change="televerserImage" style="display: none" accept=".jpg, .jpeg, .png, .gif">
			</div>
			...
			<div class="contenu" v-else-if="mode === 'recherche'">
				...
				<div class="recherche">
					<div class="rechercher">
						...
						<span role="button" tabindex="0" :title="$t('rechercher')" class="bouton-secondaire" @click="rechercher(1)" @keydown.enter="rechercher(1)"><i class="material-icons">search</i></span>
					</div>
					<div class="resultats" v-if="Object.keys(resultats).length > 0">
						<div class="item" role="button" tabindex="0" :data-w="resultat.previewWidth" :data-h="resultat.previewHeight" v-for="(resultat, index) in resultats.hits" @click="selectionnerImage(resultat.previewURL.replace('_150', '_1280'))" @keydown.enter="selectionnerImage(resultat.previewURL.replace('_150', '_1280'))" :key="'image_' + index">
							<img :src="resultat.previewURL" :alt="'Image_' + index">
						</div>
					</div>
					<div class="navigation" v-if="Object.keys(resultats).length > 0 && (page > 1 || page < (resultats.total / 12))">
						<span role="button" tabindex="0" :class="{'invisible': page < 2}" @click="modifierPage('precedente')" @keydown.enter="modifierPage('precedente')">{{ $t('pagePrecedente') }}</span>
						<span role="button" tabindex="0" :class="{'invisible': page >= (resultats.total / 12)}" @click="modifierPage('suivante')" @keydown.enter="modifierPage('suivante')">{{ $t('pageSuivante') }}</span>
					</div>
				</div>
			</div>
		...
		if (this.mode === 'edition' && this.statut !== 'min' && this.requete === '') {
			setTimeout(function () {
				if (this.pixabayAPIKey !== '') {
					...
					document.querySelector('label[for="selectionner_image_' + this.id + '"]').focus()
				}
			}.bind(this), 0)
...
<style>
.panneau .panneau-image.mode-lecture {
