	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-rebours">
			<div class="contenu" v-if="mode === 'edition'">
				<div class="rebours edition">
					<div class="heures" v-if="affichageHeures === 'oui'">
						<label :for="'heures_' + id">{{ $t('heures') }}</label>
						<input :id="'heures_' + id" type="number" :value="heures" :min="0" :max="23" @input="heures = $event.target.value">
					</div>
					<div class="minutes">
						<label :for="'minutes_' + id">{{ $t('minutes') }}</label>
						<input :id="'minutes_' + id" type="number" :value="minutes" :min="0" @input="minutes = $event.target.value">
					</div>
					<div class="secondes">
						<label :for="'secondes_' + id">{{ $t('secondes') }}</label>
						<input :id="'secondes_' + id" type="number" :value="secondes" :min="0" :max="59" @input="secondes = $event.target.value">
					</div>
				</div>
				<div class="options">
					<div class="conteneur-affichage-heures">
						...
						<div class="affichage-heures" >
							<span class="oui">
								<input :id="'heures_oui_' + id" type="radio" :name="'heures_oui_' + id" value="oui" :checked="affichageHeures === 'oui'" @change="modifierAffichageHeures($event.target.value)">
								<label :for="'heures_oui_' + id" tabindex="0" @keydown.enter="activerInput('heures_oui_' + id)">{{ $t('oui') }}</label>
							</span>
							<span class="non">
								<input :id="'heures_non_' + id" type="radio" :name="'heures_oui_' + id" value="non" :checked="affichageHeures === 'non'" @change="modifierAffichageHeures($event.target.value)">
								<label :for="'heures_non_' + id" tabindex="0" @keydown.enter="activerInput('heures_non_' + id)">{{ $t('non') }}</label>
							</span>
						</div>
					</div>
					<div class="conteneur-affichage-cercle">
						...
						<div class="affichage-cercle" >
							<span class="oui">
								<input :id="'cercle_oui_' + id" type="radio" :name="'cercle_oui_' + id" value="oui" :checked="affichageCercle === 'oui'" @change="modifierAffichageCercle($event.target.value)">
								<label :for="'cercle_oui_' + id" tabindex="0" @keydown.enter="activerInput('cercle_oui_' + id)">{{ $t('oui') }}</label>
							</span>
							<span class="non">
								<input :id="'cercle_non_' + id" type="radio" :name="'cercle_oui_' + id" value="non" :checked="affichageCercle === 'non'" @change="modifierAffichageCercle($event.target.value)">
								<label :for="'cercle_non_' + id" tabindex="0" @keydown.enter="activerInput('cercle_non_' + id)">{{ $t('non') }}</label>
							</span>
						</div>
					</div>
					<div class="conteneur-activation-effets-sonores">
						...
						<div class="effets-sonores">
							<span class="oui">
								<input :id="'son_oui_' + id" type="radio" :name="'son_' + id" value="oui" :checked="effetsSonores === 'oui'" @change="effetsSonores = $event.target.value">
								<label :for="'son_oui_' + id" tabindex="0" @keydown.enter="activerInput('son_oui_' + id)">{{ $t('oui') }}</label>
							</span>
							<span class="non">
								<input :id="'son_non_' + id" type="radio" :name="'son_' + id" value="non" :checked="effetsSonores === 'non'" @change="effetsSonores = $event.target.value">
								<label :for="'son_non_' + id" tabindex="0" @keydown.enter="activerInput('son_non_' + id)">{{ $t('non') }}</label>
							</span>
						</div>
					</div>
					<div class="conteneur-sons" v-if="effetsSonores === 'oui'">
						<label :for="'son_' + id">{{ $t('son') }}</label>
						<select :id="'son_' + id" @change="son = $event.target.value">
							<option :selected="son === 'bip'" value="bip">{{ $t('bip') }}</option>
							<option :selected="son === 'cartoon'" value="cartoon">{{ $t('cartoon') }}</option>
							<option :selected="son === 'clave'" value="clave">{{ $t('clave') }}</option>
							<option :selected="son === 'djembe'" value="djembe">{{ $t('djembe') }}</option>
							<option :selected="son === 'goutte'" value="goutte">{{ $t('gouttes') }}</option>
							<option :selected="son === 'maraca'" value="maraca">{{ $t('maraca') }}</option>
							<option :selected="son === 'mbira'" value="mbira">{{ $t('mbira') }}</option>
							<option :selected="son === 'metronome'" value="metronome">{{ $t('metronome') }}</option>
							<option :selected="son === 'triangle'" value="triangle">{{ $t('triangle') }}</option>
						</select>
					</div>
				</div>
				<div class="actions">
			...
			<div class="contenu" v-else>
				...
				<div class="actions">
					<span class="supprimer" role="button" tabindex="0" :class="{'avec-focus': statut === 'focus'}" @click="supprimer" @keydown.enter="supprimer" v-if="!tempsEcoule && (mode === 'decompte' || mode === 'pause')">{{ $t('moins1Min') }}</span>
					<span class="bouton danger" v-if="tempsEcoule">{{ $t('tempsEcoule') }}</span>
					<span class="bouton redemarrer" role="button" tabindex="0" @click="redemarrer" @keydown.enter="redemarrer" v-if="tempsEcoule && mode === 'lecture'">{{ $t('redemarrer') }}</span>
					<span class="bouton demarrer" role="button" tabindex="0" @click="demarrer(0)" @keydown.enter="demarrer(0)" v-else-if="!tempsEcoule && mode === 'lecture'">{{ $t('demarrer') }}</span>
					<span class="bouton pause" role="button" tabindex="0" @click="pause" @keydown.enter="pause" v-else-if="!tempsEcoule && mode === 'decompte'"><i class="material-icons">pause</i></span>
					<span class="bouton continuer" role="button" tabindex="0" @click="continuer" @keydown.enter="continuer" v-else-if="!tempsEcoule && mode === 'pause'"><i class="material-icons">play_arrow</i></span>
					<span class="bouton" role="button" tabindex="0" @click="redemarrer" @keydown.enter="redemarrer" v-if="!tempsEcoule && (mode === 'decompte' || mode === 'pause')"><i class="material-icons">autorenew</i></span>
					<span class="ajouter" role="button" tabindex="0" :class="{'avec-focus': statut === 'focus'}" @click="ajouter" @keydown.enter="ajouter" v-if="!tempsEcoule && (mode === 'decompte' || mode === 'pause')">{{ $t('plus1Min') }}</span>
				</div>
			</div>
		</div>
...
export default {
	...
	watch: {
		export: function (valeur) {
			if (valeur === true) {
				...
				this.$emit('export', { id: this.id, titre: this.titre, mode: this.mode, statut: this.statut, dimensions: this.dimensions, position: this.position, contenu: { heures: this.heures, minutes: this.minutes, secondes: this.secondes, affichageHeures: this.affichageHeures, affichageCercle: this.affichageCercle, effetsSonores: this.effetsSonores, son: this.son }, w: this.w, h: this.h, x: this.x, y: this.y })
			}
		}
	},
	created () {
		...
		if (this.panneau.contenu !== '') {
			...
			if (this.panneau.contenu.hasOwnProperty('son')) {
	...
	mounted () {
		...
		if (this.mode === 'edition' && this.statut !== 'min' && this.minutes === 0 && this.affichageHeures === 'oui') {
			setTimeout(function () {
				document.querySelector('#' + this.id + ' .heures input').focus()
			}.bind(this), 0)
		} else if (this.mode === 'edition' && this.statut !== 'min' && this.minutes === 0 && this.affichageHeures === 'non') {
			setTimeout(function () {
	...
	methods: {
		generer () {
			if (this.heures >= 1 || this.minutes >= 1 || this.secondes >= 1) {
				...
				this.dureeMs = (((parseInt(this.heures) * 3600) + (parseInt(this.minutes) * 60) + parseInt(this.secondes)) * 1000) - 1000
		...
		demarrer (laps) {
			...
			this.mode = 'decompte'
			this.$nextTick(function () {
				document.querySelector('#' + this.id + ' .bouton.pause').focus()
			}.bind(this))
		},
		redemarrer () {
			if (this.decompte !== '') {
			...
			this.dureeMs = (((parseInt(this.heures) * 3600) + (parseInt(this.minutes) * 60) + parseInt(this.secondes)) * 1000)
			this.$nextTick(function () {
		...
		decompter () {
			...
			if (temps.total <= 10000 && temps.total >= 0 && this.effetsSonores === 'oui') {
				const audio = document.querySelector('#audio-' + this.son)
				audio.play()
			}
			if (temps.total <= 0) {
				clearInterval(this.decompte)
		...
		pause () {
			...
			this.$nextTick(function () {
				document.querySelector('#' + this.id + ' .bouton.continuer').focus()
			}.bind(this))
		},
		continuer () {
		...
		ajouter () {
			...
			this.dureeMs = this.dureeMs + 60000
		...
		supprimer () {
			...
			if (temps > 60000) {
				...
				const tempsRestant = temps - 60000
				if (parseInt(this.texteMinutes) > 0) {
					this.texteMinutes = parseInt(this.texteMinutes) - 1
				} else if (parseInt(this.texteMinutes) === 0 && this.affichageHeures === 'oui') {
					this.texteMinutes = '00'
					this.texteHeures = parseInt(this.texteHeures) - 1
				}
				if (parseInt(this.texteSecondes) < 10) {
...
.rebours.edition + .options .conteneur-sons,
...
.rebours.edition + .options .conteneur-sons select {
	height: 21px;
	padding: 0 1.5rem;
	line-height: 1;
}
...
.panneau-rebours span.avec-focus {
	color: #fff;
}
