		...
		<div class="conteneur panneau-nuage">
			<div class="contenu" v-if="mode === 'edition'">
				<div class="colonne gauche">
					...
					<span class="bouton-secondaire" role="button" tabindex="0" :title="$t('ajouterMot')" @click="ajouterMot" @keydown.enter="ajouterMot"><i class="material-icons">add_circle_outline</i></span>
					<span class="bouton-secondaire bouton-liste" role="button" tabindex="0" :title="$t('ajouterListe')" @click="afficherListe" @keydown.enter="afficherListe"><i class="material-icons">list_alt</i></span>
				</div>
				<div class="colonne droite">
					...
					<div class="actions">
						<span class="bouton" role="button" tabindex="0" @click="visualiser" @keydown.enter="visualiser">{{ $t('apercu') }}</span>
				...
				<Transition name="fondu">
					<div class="liste" v-if="modale">
						...
						<div class="actions">
							...
							<span class="bouton" role="button" tabindex="0" @click="ajouterMots" @keydown.enter="ajouterMots">{{ $t('ajouter') }}</span>
						</div>
					</div>
				</Transition>
			</div>
	...
	methods: {
		...
		ajouterMot () {
			...
			this.$nextTick(function () {
				...
				const mots = document.querySelectorAll('#' + this.id + ' .mots .mot')
				for (let i = 0; i < mots.length; i++) {
					if (i === this.mots.length - 1) {
						mots[i].querySelector('input').focus()
					}
				}
			})
		},
		...
		fermerListe () {
			this.modale = false
			this.$nextTick(function () {
				document.querySelector('#' + this.id + ' .bouton-liste').focus()
			}.bind(this))
		}
	}
}
...
.panneau .panneau-nuage .liste textarea {
	max-height: 18rem;
	height: 18rem;
}
