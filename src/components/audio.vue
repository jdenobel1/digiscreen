	...
	<JsPanel class="panneau" :visible="!chargement" :options="options">
		...
		<div class="conteneur panneau-audio">
			<div class="contenu" v-if="mode === 'edition' && !chargementAudio">
				...
				<div class="choix">
					<span class="lecteur-audio">
						<input :id="'lecteur_' + id" type="radio" :name="'type_' + id" value="lecteur" :checked="type === 'lecteur'" @change="type = $event.target.value">
						<label :for="'lecteur_' + id" tabindex="0" @keydown.enter="activerInput('lecteur_' + id)">{{ $t('lecteur') }}</label>
					</span>
					<span class="bouton-audio">
						<input :id="'bouton_' + id" type="radio" :name="'type_' + id" value="bouton" :checked="type === 'bouton'" @change="type = $event.target.value">
						<label :for="'bouton_' + id" tabindex="0" @keydown.enter="activerInput('bouton_' + id)">{{ $t('bouton') }}</label>
					</span>
				</div>
				<label>{{ $t('lienFichierAudio') }}</label>
				<div class="valider">
					<input type="search" :value="audio" @input="audio = $event.target.value" @keydown.enter="valider">
					<span role="button" tabindex="0" :title="$t('valider')" class="bouton-secondaire" @click="valider" @keydown.enter="valider"><i class="material-icons">search</i></span>
				</div>
				<div class="separateur"><span>{{ $t('ou') }}</span></div>
				<label>{{ $t('fichierAppareil') }}</label>
				<label :for="'selectionner_audio_' + id" class="bouton" role="button" tabindex="0" @keydown.enter="activerInput('selectionner_audio_' + id)">{{ $t('selectionnerAudio') }}</label>
				<input :id="'selectionner_audio_' + id" type="file" @change="televerserAudio" style="display: none" accept=".mp3, .wav, .m4a, .ogg">
			</div>
			...
			<div class="contenu lecture-audio" v-else>
				...
				<span class="bouton audio" role="button" tabindex="0" @click="lireAudio" @keydown.enter="lireAudio" v-if="type === 'bouton'">
					<i class="material-icons" v-if="!lecture">play_arrow</i>
					<i class="material-icons" v-else>pause</i>
				</span>
				<span class="bouton-secondaire" role="button" tabindex="0" :title="$t('ajouterListe')" @click="relireAudio" @keydown.enter="relireAudio" v-if="type === 'bouton'"><i class="material-icons">replay</i></span>
			</div>
		</div>
...
export default {
	...
	mounted () {
		...
		} else if (this.mode === 'edition' && this.statut !== 'min' && this.audio !== '') {
			setTimeout(function () {
				document.querySelector('#' + this.id + ' audio').focus()
			}.bind(this), 0)
